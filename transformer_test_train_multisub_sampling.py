...
trim_electrodes_to = 80 # TODO: make this a variable not always 100
...
for electrode_emb, dataloader in zip(electrode_emb_store, dataloader_store):
    for i in range(len(dataloader)):
        ...
        neg_data[:, 0, :] = neg_data[:, 0, elec_perm]
        # Shuffle time bins 
        time_perm = torch.randperm(n_time_bins)
        neg_data[:, 1, :] = neg_data[:, 1, :, time_perm]
        neg_data[:, 2, :] = torch.randn_like(neg_data[:, 2, :]) * noise_scale
